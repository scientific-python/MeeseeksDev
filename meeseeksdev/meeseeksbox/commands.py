...
def safe_backport(session, payload, arguments, local_config=None):
    ...
    try:
        ...
        labels_names = []
        try:
            labels_names = [l["name"] for l in pr_data["labels"] if l["name"] != "Still Needs Manual Backport"]
            if not labels_names and ("issue" in payload.keys()):
                labels_names = [l["name"] for l in payload["issue"]["labels"] if l["name"] != "Still Needs Manual Backport"]
        except KeyError:
